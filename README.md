...
Ok so I didn't want to let my idea just wander around the roads of theory so I built a little server with a single api endpoint and a client app that uses the data. Then I asked the powers that be at VitalSource for a gemini api key so I could make api calls to an AI framework in real time. And were going to test all this in a live demo. Im going to show you what happens to our little app if the strict contract is broken on either client or server side. Then I'm going to turn on the an api interceptor that allows AI to change the data when a contract is not upheld in hopes that we can resolve the issue. For this I'm going to need to swap back and forth between my browser and my code editor to explain some what's going on.
...
And here is our Demo app. we currently make a call to get som Person data and use that data down here to show the data we received, name and facts.birth_date. Here is where we make our api request to get the data.
...
Let's change some stuff up and see the result. We'ere going to start with the server.
...
refresh the app and of course we get an error "Cannot read properties of undefined (reading 'birth_date')" because the object no longer has a "facts" object to read from.
...
At this point we check if AI's response matches our expectations. If not we simply send back the unedited response from the server but if it does we update the response with the AI response and pass it along! If our first attempt failed in a real app, this would be a great chance to add some retry logic or even adjust the question sent to the AI API in hopes for a better response.

So lets look at what happens now when we add our updates to the server and client side with AI adjust toggled "on".

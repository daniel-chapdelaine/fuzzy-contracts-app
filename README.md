...
Ok so I didn't want to let my idea just wander around the roads of theory so I built a little server with a single api endpoint and a client app that uses the data. Then I asked the powers that be at Vitalsource for a gemini api key so that I could make api calls to an ai framework in real time. And were going to test all this right now. Im going to show you what happens to our little app if the strict contract is broken on either client or server side. Then I'm going to turn on the an api interceptor that allows AI to change the data when a contract is not upheld in hopes that we can resolve the issue.

[Introduce apps]

Here is the server, as you can see right now it simply returns the name of the user and some facts about them. Currently the only fact is birth date. And here is our Demo app. we currently make a call to get a Person object and use that object down here to show the data we recieved, name and facts.birth_date. Here is where we make our api request to get the data. This CustomAxiosRequestConfig is only used when AI adjust is on but it is currently off so no need to worry about that right now. And here is the result. When the contract is abided by we see the data on the screen.

[Walk through how the apps would react if the client side changed without the server side changes and vice versa]
Let's change some stuff up though and see the result. Let's change the server side first and just to shake things up I'm going to make the change to the contract more complex than the key name change I talked about earlier. We are going to completely remove the facts object and change birth_date to favorite_date like so... Remember this simulates what it would be like if we pushed a change to our server side code before the client side changes were made. So I'm going to save that, refresh the app and of course we get an error "Cannot read properties of undefined (reading 'birth_date')" because the object no longer has a "facts" object to read from.

So let's put that back. And now let's update the client side as if the the client were ahead of the server changes. Now instead of the Person object we will expect to see the NewPerson object with "name" and "favorite_date" only. We will also need to update the tsx or TypeScript will yell at us. Ok so this time there is a sneakier issue. The app has not shut down but our date we had before is missing! And of course it is, we are looking for key of "favorite_date" that does not exist on the data.

[Reset]

Ok so im just going to put everything back real quick. Now we can run it back but this time using AI to help us adjust the data shape if issue is detected.

[Explain the AI Adjust button]

Right here we have a little AI ajust toggle right now it is off so we wont query the Gemini API if it is off. We also won't query it if our data is in accordance to the contract's shape. So if i turn this on you can see "Nope - data looked good." for whether our data was changed. Let me explain what's happening behind the scenes before we make any data changes.

When a response is returned from the server it is intercepted by this method here. The config passes in a schema which defines the shape of the data the Client expects. If there is no schema or if we have ai adjust set to "off" we simply allow the response to flow through the app as normal. If that check is passed we look to see if the data in the response is the expected shape which is captured by this "success" constant. I wont go into to how this works bc it's not important. What is important is what happens with this contant. If the shape is good, the data is allowed to continue to the app. If it is not we let Gemini have a go at it.

This aiAdjust function is defined here. We pass in the schema and transform it into a string and add it to this statement. "Modify 'the data we received' to match 'the shape we desire' and ensure the response is valid JSON but not in a code block. If we don't specify that we don't want the code block Gemini thinks we want it formatted, which we don't. We send the question and parse the answer. Sprinkle in some metadata about what we did and then return that to our interceptor. At this point we check if AI's response matches our expectations. If not we simply send back the unedited response from the server but if it does we update the response with the AI response and pass it along! I think in a real app this would be a great chance to add some retry logic or even add logic to adjust the question sent to the AI API in hopes for a better response.

[Walk through how the apps now react with the AI logic in place]

Ok so lets look at what happens now when we add our updates to the server and client side with AI adjust toggled "on".

So once again we are going to update the server only. Last time we had an unhandled error on the client side but this time we seem to be up and running. We recognized the data was misaligned and built this question to send to Gemini. And here is the answer it gave. We were able to pass that along and continue working smoothly. Let's put that back and change the client. So now the client is going to make it's changes first. I'm going to refresh the page and same thing! Data wasn't quite right so it adjusted to the new schema and our app is working. 

And what happens when the server and client are both updated? We return to a state where we no longer look to Gemini to help us out.
